...
export default {
  ...
  async updatePosition(ctx: any) {
    try {
      ...
      const event = await strapi.entityService.create('api::vehicle-event.vehicle-event' as any, {
        data: {
          vehicle_id,
      ...
      try {
        await strapi.entityService.update('api::vehicle-status.vehicle-status' as any, vehicle_id, {
          data: {
   ...
   */
  async updatePassengerCount(ctx: any) {
    try {
   ...
   */
  async driverConfirmBoarding(ctx: any) {
    try {
      ...
      try {
        await strapi.entityService.update('api::active-passenger.active-passenger' as any, passenger_id, {
          data: {
            status: 'onboard',
   ...
   * Hardware: Driver tablet button press
   */
  async driverConfirmAlighting(ctx: any) {
    try {
      const { vehicle_id, passenger_id, stop_id, timestamp } = ctx.request.body;
