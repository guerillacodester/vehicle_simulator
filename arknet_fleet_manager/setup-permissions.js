...
setupPermissions();
    ...
    const actions = ['find', 'findOne']; // Basic read permissions
    ...
    for (const contentType of contentTypes) {
      for (const action of actions) {
        ...
        const existingPerm = await client.query(`
          ...
          WHERE action = $1 AND subject = $2 AND role = $3
        `, [action, contentType, publicRole.id]);
        ...
        if (existingPerm.rows.length === 0) {
          ...
          console.log(`✓ Added ${action} permission for ${contentType}`);
        } else {
          console.log(`• ${action} permission already exists for ${contentType}`);
        }
      }
