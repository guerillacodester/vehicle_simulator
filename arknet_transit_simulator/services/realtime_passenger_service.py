...
# -*- coding: utf-8 -*-
"""
Real-Time Passenger Generation Service

Integrates with the depot system to continuously generate passengers
for each route according to the passenger models, with real-time factors
...
@dataclass 
class PassengerGenerationConfig:
    """Configuration for real-time passenger generation."""
    generation_interval_seconds: int = 30  # How often to generate
    max_total_passengers: int = 50
...
class RealTimePassengerService:
    """Service for real-time passenger generation integrated with depot system."""
    
    def __init__(self, config: PassengerGenerationConfig = None):
        self.config = config or PassengerGenerationConfig()
        ...
        try:
            from arknet_transit_simulator.services.passenger_model_loader import PassengerModelLoader
            from arknet_transit_simulator.services.passenger_generation_engine import PassengerGenerationEngine
            
            self.model_loader = PassengerModelLoader()
            self.generation_engine = PassengerGenerationEngine(self.model_loader)
            self.generation_engine.set_passenger_limit(self.config.max_total_passengers)
            
            # Load Route 1/1A/1B model
            model_loaded = self.model_loader.load_model("route_1_1a_1b_model.json")
            if model_loaded:
                logger.info("✅ Loaded Route 1/1A/1B passenger model for real-time generation")
            else:
                logger.warning("⚠️ Could not load passenger model - using fallback generation")
            
            self.components_available = True
            
        except ImportError as e:
            logger.warning(f"⚠️ Passenger generation components not available: {e}")
    ...
    def register_route(self, route_id: str, route_name: str, stops: List[str], frequency_minutes: int) -> bool:
        """Register a route for real-time passenger generation."""
        try:
            route = ActiveRoute(
                route_id=route_id,
    ...
    def start_real_time_generation(self) -> bool:
        """Start real-time passenger generation for all registered routes."""
        ...
        logger.info("🚀 Starting real-time passenger generation service")
    ...
    def stop_real_time_generation(self) -> None:
        """Stop real-time passenger generation."""
        logger.info("🛑 Stopping real-time passenger generation service")
        self.is_running = False
        ...
        logger.info("✅ Real-time passenger generation stopped")
    ...
    async def _route_generation_loop(self, route_id: str) -> None:
        ...
        try:
            while self.is_running and route.is_active:
                # Generate passengers for each stop on this route
                await self._generate_route_passengers(route)
    ...
    async def _generate_route_passengers(self, route: ActiveRoute) -> None:
        """Generate passengers for all stops on a route."""
        ...
        if self.config.debug_logging:
            logger.debug(f"🎯 Generating passengers for route {route.route_id} at {current_time.strftime('%H:%M:%S')}")
        ...
        for stop_id in priority_stops:
            try:
                ...
                if self.current_passengers >= self.config.max_total_passengers:
                    if self.config.debug_logging:
                        logger.debug(f"⚖️ Skipping generation - at capacity limit ({self.current_passengers}/{self.config.max_total_passengers})")
                    break
                
                # Generate passenger batch for this stop
                ...
                if batch and batch.total_boarding > 0:
                    ...
                    self.current_passengers += batch.total_boarding
                    
                    if self.config.debug_logging:
                        logger.debug(f"👥 Generated {batch.total_boarding} passengers at {stop_id}")
                    
                    # Notify depot system (this would be the integration point)
                    await self._notify_depot_system(route.route_id, stop_id, batch)
                
            except Exception as e:
                logger.error(f"❌ Error generating passengers for {stop_id}: {e}")
        
        if total_generated > 0:
            logger.info(f"🚌 Route {route.route_id}: Generated {total_generated} passengers across {len(priority_stops)} stops")
    
    async def _notify_depot_system(self, route_id: str, stop_id: str, batch) -> None:
        """Notify the depot system about newly generated passengers."""
        # This is where you would integrate with your actual depot system
        # For now, just log the notification
        
        passenger_details = []
        for passenger in batch.boarding_passengers[:3]:  # Show first 3
            passenger_details.append(f"{passenger.trip_purpose} (wait: {passenger.wait_time_seconds//60}min)")
        
        logger.info(f"📞 DEPOT NOTIFICATION: Route {route_id} @ {stop_id}")
        logger.info(f"   👥 {batch.total_boarding} new passengers: {', '.join(passenger_details)}")
        
        # Here you would call your depot system's passenger update method
        # Example: depot_system.add_passengers(route_id, stop_id, batch.boarding_passengers)
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_passengers": self.config.max_total_passengers,
            "capacity_utilization": (self.current_passengers / self.config.max_total_passengers) * 100,
    ...
    def update_passenger_count(self, change: int) -> None:
        """Update current passenger count (when passengers board/alight)."""
        self.current_passengers = max(0, self.current_passengers + change)
        if self.config.debug_logging:
            logger.debug(f"📊 Passenger count updated: {self.current_passengers}")
...
async def example_depot_integration():
    ...
    service = RealTimePassengerService(config)
    ...
    if service.start_real_time_generation():
        print("🚀 Real-time passenger generation started!")
        ...
        try:
            for i in range(10):  # Run for 10 cycles
                ...
                print(f"\n📊 Status Update #{i+1}:")
                print(f"   Active passengers: {status['current_passengers']}/{status['max_passengers']}")
                print(f"   Capacity: {status['capacity_utilization']:.1f}%")
                print(f"   Active tasks: {status['active_generation_tasks']}")
                
                # Simulate passengers boarding/alighting
                if i % 3 == 0:  # Every 3rd cycle, some passengers leave
                    passengers_leaving = min(5, status['current_passengers'])
                    service.update_passenger_count(-passengers_leaving)
                    print(f"   🚪 {passengers_leaving} passengers alighted")
        ...
        print("✅ Real-time passenger generation stopped")
    
    else:
        print("❌ Failed to start real-time passenger generation")
