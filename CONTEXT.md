...
**Files Modified:**
- `commuter_service/interfaces/http/passenger_crud.py`
  - Added `/api/passengers/nearby` endpoint
  - Accepts lat, lon, radius parameters
  - Filters passengers using Python haversine
  - Returns passengers sorted by distance

- `arknet_transit_simulator/services/commuter_http_client.py` (NEW)
  - Created `CommuterServiceClient` class
  - Methods: `connect()`, `get_eligible_passengers()`, `board_passenger()`, `alight_passenger()`
  - Replaces direct database access

- `arknet_transit_simulator/vehicle/conductor.py`
  - Changed constructor: `passenger_db` → `commuter_service_url`
  - `_start_implementation()` initializes CommuterServiceClient
  - `_stop_implementation()` disconnects HTTP client
  - `check_for_passengers()` uses HTTP client

- `arknet_transit_simulator/simulator.py`
  - Removed PassengerRepository/StrapiPassengerService injection
  - Passes `commuter_service_url` from config to conductor

**Architecture Now:**
```
Conductor → CommuterServiceClient → HTTP API → (Future: Reservoir) → Repository → Strapi
```

### 2. Strict DB-Bound Configuration (Partial) ✅
**Completed:**
- `ConductorConfig.pickup_radius_km` changed from `float = 0.2` to `Optional[float] = None`
- Added `ConductorConfig.validate()` method that raises error if pickup_radius_km is None
- Forces proper database configuration, no silent fallbacks

**Remaining:**
- Update `ConductorConfig.from_config_service()` to remove hardcoded fallback
- Call `validate()` after loading from config service
- Test startup failure when config missing

### 3. GPS Telemetry Safety Verification ✅
**Verified Flow:**
- `Driver.step()` → `telemetry_buffer.write(telemetry)` → GPS Device → GPSCentCom (port 5000)
- Conductor position updates are parallel: `update_vehicle_position(lat, lon)`
- **Conclusion:** GPS telemetry flow is separate and safe from conductor changes

### 4. Coordinate Validation Decision ✅
**Analysis:**
- Route coordinates: GTFS → Strapi → Fleet Manager API → Dispatcher → Driver
- `interpolate_along_route_geodesic()` only uses pre-validated route coordinates
- Pure deterministic math: valid input → valid output
- **Decision:** No validation needed in `update_vehicle_position()` - coordinates from controlled flow

**Findings:**
- GTFS data is industry-standard validated format
- No external inputs, no user data, no network calls in interpolation
- Function can only fail by crashing (math errors), not silently producing invalid coords

## Current State Analysis

### Performance Critical Issue ⚠️
**Problem:** Conductor performs real-time passenger searches during operation
- Current: Fetches ALL passengers from Strapi, filters in Python (O(n))
- With 1,000 passengers: ~500ms per search
- With 10,000 passengers: ~5,000ms (5 seconds!) - UNUSABLE

**Solution:** Phase 4 (PostGIS integration) is CRITICAL
- Database spatial index: O(log n)
- Expected performance: <15ms even with 10,000 passengers
- 416x speedup at scale

### Data Flow (Verified)
```
GTFS Data (validated)
  ↓
Strapi Database (route_shapes, shapes tables)
  ↓
Fleet Manager API (/api/v1/routes/public/{route_code}/geometry)
  ↓
Dispatcher.get_route_info()
  ↓
Driver.route (List[Tuple[float, float]])
  ↓
Driver.step() → interpolate_along_route_geodesic()
  ↓
Telemetry {lat, lon, speed, heading}
  ├→ GPS Device → GPSCentCom (port 5000) [separate flow]
  └→ Conductor.update_vehicle_position(lat, lon) [position awareness]
```

### Geospatial Functions (Duplicated)
**Locations:**
1. `arknet_transit_simulator/vehicle/conductor.py` - haversine()
2. `arknet_transit_simulator/vehicle/driver/navigation/math.py` - haversine(), bearing(), forward_point()
3. `commuter_service/interfaces/http/passenger_crud.py` - haversine()
4. Potentially more scattered across codebase

**Issue:** Code duplication, inconsistent implementations
**Solution:** Phase 1 consolidates into shared `utils/geospatial.py`

## Implementation Plan (7 Phases - 3 Hours Total)

### Phase 1: Shared Geospatial Utilities ⏱️ 30 min
- Create `arknet_transit_simulator/utils/geospatial.py`
- Consolidate: `haversine()`, `bearing()`, `forward_point()`
- Update imports in all files using duplicates

### Phase 2: Complete DB-Bound Configuration ⏱️ 15 min
- Update `ConductorConfig.from_config_service()` to remove `pickup_radius_km or 0.2`
- Call `config.validate()` after loading
- Test startup failure if config missing

### Phase 4: PostGIS Integration (CRITICAL) ⏱️ 60 min
**Priority:** Performance-critical for real-time passenger searches

**Current State:**
```python
# commuter_service/interfaces/http/passenger_crud.py
# Fetches ALL passengers, filters in Python
passengers = await strapi_client.get_all_passengers()
for p in passengers:
...
**Target State:**
```python
# Use geospatial_service PostGIS
# Database returns only nearby passengers (spatial index)
nearby = await geospatial_service.get_passengers_within_radius(lat, lon, radius)
```

**Changes:**
1. Create `/api/spatial/passengers/within-radius` in geospatial_service
2. Use PostGIS query: `ST_DWithin(geography, point, radius)`
3. Update `/api/passengers/nearby` to call geospatial_service
...
| 100       | 50ms           | 5ms             | 10x     |
| 1,000     | 500ms          | 8ms             | 62x     |
| 10,000    | 5,000ms        | 12ms            | 416x    |

### Phase 5: Route Geometry Awareness ⏱️ 45 min
**Current:** Conductor only knows current lat/lon point, searches circular radius
**Target:** Conductor aware of route geometry, searches along route corridor

**Changes:**
1. Add `route_geometry: Optional[List[Tuple[float, float]]]` to conductor
2. Driver passes route to conductor on initialization
3. Update `update_vehicle_position()` to store route context
4. Enhance passenger queries: PostGIS `ST_Distance(linestring, point)`

**Benefit:** Avoid picking up passengers off-route (e.g., opposite side of highway/river)

### Phase 6: Real-Time Configuration Updates ⏱️ 30 min
**Current:** `pickup_radius_km` loaded once on startup, requires restart to change
**Target:** WebSocket listener for config changes, update dynamically

**Changes:**
1. Add WebSocket client in conductor for Strapi config updates
2. Subscribe to `operational-configuration` update events
3. Update `self.pickup_radius_km` on change
4. Log config changes

**Benefit:** Operators tune pickup radius without restarting fleet

### Phase 7: Integration Testing ⏱️ 30 min
**Test Scenarios:**
- ✅ GPS telemetry still flows (verified safe)
- 🧪 Conductor starts with DB config
- 🧪 Conductor fails fast if config missing
- 🧪 Passenger detection via HTTP API
- 🧪 PostGIS queries correct results
- 🧪 Route geometry filtering works
- 🧪 Dynamic config updates apply

### Phase 8: Documentation Cleanup ⏱️ 15 min
- Update `arknet_transit_simulator/README.md`
- Update `commuter_service/README.md`
- Add docstrings to geospatial utilities
- Document PostGIS integration

## Removed from Plan
**Phase 3: Position Validation** ❌
- **Reason:** Coordinates from validated GTFS in closed loop
- **Analysis:** Interpolation is pure deterministic math on pre-validated data
- **Decision:** No validation needed - unnecessary overhead
- **Time Saved:** 45 minutes

## Technical Details

### Haversine Formula (Current Implementations)
```python
...
- `ConductorConfig.validate()` may fail on startup if DB config missing (DESIRED)
- API signature changes internal only (HTTP client abstracts)
- No changes to GPS telemetry flow

### Dependencies
- **geospatial_service** must be running (port 6000)
- **Strapi** must have passengers table with lat/lon columns
- **PostGIS** extension enabled in database
- **commuter_service** must be running (port 4000)

### Success Criteria
1. Conductor starts successfully with DB config
2. Conductor fails fast if config missing
3. Passenger searches <15ms with 10,000 passengers
4. GPS telemetry continues flowing to GPSCentCom
5. Route-aware passenger filtering works correctly
6. Dynamic config updates apply without restart

## Next Steps
1. Start with Phase 1 (shared geospatial utilities) - foundation for all other phases
2. Complete Phase 2 (DB-bound config) - quick win
3. **Priority:** Phase 4 (PostGIS) - critical performance improvement
4. Phase 5 (route geometry) - precision improvement
5. Phase 6 (real-time config) - operational flexibility
6. Phase 7 (integration testing) - validation
7. Phase 8 (documentation) - knowledge transfer

## Timeline
- **Total:** 3 hours (down from 4.5 hours after removing validation phase)
- **Target:** Complete today (November 4, 2025)
- **Achievable:** Yes, with focused execution
