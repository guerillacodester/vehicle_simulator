...
Usage:
    python commuter_service/seed_passengers.py --day monday --route all
    python commuter_service/seed_passengers.py --day monday --route 1
    python commuter_service/seed_passengers.py --day monday --route 1 --depot-spawning
"""
...
async def seed_passengers(day: str, route_filter: str, enable_depot: bool = False):
    ...
    if route_filter.lower() == 'all':
        ...
        selected_routes = [r for r in routes if r.short_name == route_filter]
        if not selected_routes:
            print(f"ERROR: Route '{route_filter}' not found!")
            print(f"Available routes: {', '.join([r.short_name for r in routes])}")
            await passenger_repo.disconnect()
            return
        print(f"📍 Seeding Route {selected_routes[0].short_name} - {selected_routes[0].long_name}")
    ...
    for route in selected_routes:
        ...
        logger.info(f"✅ RouteSpawner created for Route {route.short_name} - {route.long_name}")
    ...
    if enable_depot:
        ...
        for depot in depots:
            depot_reservoir = DepotReservoir(
                depot_id=depot.document_id,
                passenger_repository=passenger_repo,
                enable_redis_cache=False
            )
            
            # Get depot location
            if depot.location and hasattr(depot.location, 'coordinates') and len(depot.location.coordinates) == 2:
                depot_location = (depot.location.coordinates[1], depot.location.coordinates[0])  # (lat, lon)
            else:
                logger.warning(f"Skipping depot {depot.document_id} - no valid location")
                continue
            ...
            spawner = DepotSpawner(
                ...
                available_routes=[r.document_id for r in selected_routes],
                depot_document_id=depot.document_id,
                config_loader=config_loader,
                geo_client=geo_client
            )
            depot_spawners.append((depot, spawner, depot_reservoir))
            logger.info(f"✅ DepotSpawner created for: {depot.name}")
        ...
        for route, spawner, reservoir in route_spawners:
            try:
                ...
                if spawn_requests:
                    ...
                    if failed > 0:
                        logger.warning(f"Route {route.short_name} hour {hour}: {failed} failed")
            ...
            for depot, spawner, reservoir in depot_spawners:
                try:
                    ...
                    if spawn_requests:
                        ...
                        if failed > 0:
                            logger.warning(f"Depot {depot.name} hour {hour}: {failed} failed")
                except Exception as e:
