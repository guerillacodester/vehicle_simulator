...
import psycopg2
from psycopg2.extras import RealDictCurs    if error_count == 0:
        print("✅ All GPS devices migrated successfully!")
    else:
        print(f"⚠️  {error_count} GPS devices failed to migrate")
    
    finally:
        # Clean up SSH tunnel
        print("Closing SSH tunnel...")
        tunnel.stop()
        print("✅ SSH tunnel closed")import requests
import json
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for device in data['data']:
                # Use device_uuid as unique identifier
                if 'device_uuid' in device['attributes']:
                    existing[device['attributes']['device_uuid']] = device
            return existing
        else:
...
def create_strapi_gps_device(remote_device):
    ...
    strapi_data = {
        "data": {
            "device_uuid": str(remote_device['device_id']),  # Map UUID to string
            "device_serial": remote_device.get('device_serial'),
            "device_model": remote_device.get('device_model'),
            "firmware_version": remote_device.get('firmware_version'),
            "last_communication": remote_device.get('last_communication'),
            "battery_level": remote_device.get('battery_level'),
            "signal_strength": remote_device.get('signal_strength'),
            "location_accuracy": remote_device.get('location_accuracy'),
            "status": remote_device.get('status', 'unknown'),
            "installation_date": remote_device.get('installation_date'),
            "warranty_expiry": remote_device.get('warranty_expiry'),
...
def migrate_gps_devices():
    ...
    for i, device in enumerate(remote_devices, 1):
        ...
        print(f"[{i}/{len(remote_devices)}] Processing {device_serial} ({device_uuid})")
