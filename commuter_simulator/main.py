...
# Add project root to path    python commuter_simulator/main.py --day monday --route 1    python commuter_simulator/main.py --day monday --route 1

sys.path.insert(0, str(Path(__file__).parent.parent))

    python commuter_simulator/main.py --day monday --route all --depot-spawning    python commuter_simulator/main.py --day monday --route all --depot-spawning

from commuter_simulator.core.application.coordinators import SpawnerCoordinator

from commuter_simulator.core.domain.services.spawning import DepotSpawner, RouteSpawner, SpawnerInterface, SpawnRequest""""""

from commuter_simulator.core.domain.services.reservoirs import RouteReservoir, DepotReservoir
...
from commuter_simulator.infrastructure.geospatial.client import GeospatialClientimport asyncioimport asyncio
...
# Add project root to path# Add project root to path
    ...
    return {
        ...
        # Redis cache (infrastructure setting)from commuter_simulator.core.domain.spawner_engine.route_spawner import RouteSpawnerfrom commuter_simulator.core.domain.spawner_engine.route_spawner import RouteSpawner

        'enable_redis_cache': config.getboolean('commuter_simulator', 'enable_redis_cache', fallback=False),

    }from commuter_simulator.core.domain.spawner_engine.depot_spawner import DepotSpawnerfrom commuter_simulator.core.domain.spawner_engine.depot_spawner import DepotSpawner



from commuter_simulator.domain.services.reservoirs.route_reservoir import RouteReservoirfrom commuter_simulator.domain.services.reservoirs.route_reservoir import RouteReservoir

async def load_operational_config(config_service: ConfigurationService) -> dict:

    """Load operational configuration from database (operational-configurations)"""from commuter_simulator.domain.services.reservoirs.depot_reservoir import DepotReservoirfrom commuter_simulator.domain.services.reservoirs.depot_reservoir import DepotReservoir

    return {

        'continuous_mode': await config_service.get('passenger_spawning.operational.continuous_mode', default=True),from commuter_simulator.infrastructure.config.spawn_config_loader import SpawnConfigLoaderfrom commuter_simulator.infrastructure.config.spawn_config_loader import SpawnConfigLoader

        'spawn_interval_seconds': await config_service.get('passenger_spawning.operational.spawn_interval_seconds', default=60),

        'enable_routespawner': await config_service.get('passenger_spawning.operational.enable_routespawner', default=True),from commuter_simulator.infrastructure.geospatial.client import GeospatialClientfrom commuter_simulator.infrastructure.geospatial.client import GeospatialClient

        'enable_depotspawner': await config_service.get('passenger_spawning.operational.enable_depotspawner', default=True),

    }from commuter_simulator.infrastructure.persistence.strapi.passenger_repository import PassengerRepositoryfrom commuter_simulator.infrastructure.persistence.strapi.passenger_repository import PassengerRepository
...
async def main():
    ...
    logger.info(f"🗺️  Geospatial: port {infra_config['geospatial_port']}")    'saturday': datetime(2024, 11, 2),    'saturday': datetime(2024, 11, 2),
    ...
    logger.info("⚙️  ConfigurationService initialized")
    ...
    logger.info(f"⏱️  Spawn interval: {operational_config['spawn_interval_seconds']}s")async def main():async def main():
    ...
    parser.add_argument('--depot-spawning', action='store_true',    parser.add_argument('--depot-spawning', action='store_true',
        ...
        logger.info("🗄️  [3/5] Creating reservoirs...")
    ...
    # Filter routes    config = {**infra_config, **operational_config}
        ...
        if not spawners:
            ...
            logger.warning("⚠️  No spawners created! Check enable flags and database content.")    if args.route == 'all':    
        ...
        logger.info(f"⏱️  Spawn interval: {spawn_interval_seconds}s = {time_window_minutes:.4f} minutes per cycle")            print(f"Available: {', '.join([r.get('short_name', '?') for r in routes])}")        
...
if __name__ == "__main__":
    ...
    route_spawners = []            logger.warning("⚠️  No active routes or depots found in database!")
    ...
    for hour in range(24):        logger.info(f" ⏱️  Spawn interval: {spawn_interval_seconds}s = {time_window_minutes:.4f} minutes per cycle")
