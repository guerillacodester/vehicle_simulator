#!/usr/bin/env python3
"""Test admin authenticated SSE telemetry access"""
import requests
import json
import time

print("=" * 70)
print("Testing ADMIN (david) Authenticated Telemetry Access via SSE")
print("=" * 70)

# Test with admin authentication
print("\nTesting AUTHENTICATED access (admin: david)...")
try:
    # Login to get JWT cookie
    print("Logging in as david (admin)...")
    login_response = requests.post(
        'http://localhost:7000/login',
        json={'username': 'david', 'password': 'Ga25w123'}
    )
    ...
    if login_response.status_code == 200:
        ...
        if response.status_code == 200:
            print("[OK] Connected to SSE stream with admin authentication")
                    ...
                    if decoded.startswith('data:'):
                        ...
                        try:
                            ...
                            if obj.get('type') == 'update':
                                ...
                                if 'extras' in state:
                                    print(f"  [ADMIN FIELD] extras: {state.get('extras')}")
                                print()
                        except Exception as e:
                            print(f"  Parse error: {e}")
            
            print(f"\n[OK] Received {count} telemetry updates as admin")
            print("[OK] Admin should see ALL fields including extras and internal metrics")
        else:
