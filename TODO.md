...
**Current Phase**: PostGIS Integration for Real-Time Passenger Queries  
**Estimated Completion**: 3 hours (7 phases remaining)
...
TIER 4.9: Conductor Spatial Awareness & Performance Optimization 🎯 IN PROGRESS (Nov 4, 2025)
  ...
  **Problem Statement:**
  Conductor was bypassing reservoir pattern, using deprecated passenger_db methods. Additionally:
  - Performance: Real-time passenger searches use O(n) Python filtering (unusable at scale: 5s with 10k passengers)
  - Architecture: Direct database access instead of HTTP API → Reservoir pattern
  - Configuration: Hardcoded proximity radius fallback (0.2 km) instead of DB-bound
  - Spatial: No route geometry awareness (circular radius vs corridor search)
  
  **Solution:** HTTP API + PostGIS + Route Geometry Awareness
  
  **Completed:**
  - ✅ Created /api/passengers/nearby HTTP endpoint in commuter_service
  - ✅ Created CommuterServiceClient for conductor (replaces direct DB access)
  - ✅ Refactored conductor to use commuter_service_url instead of passenger_db
  - ✅ Updated simulator to pass HTTP URL to conductor
  - ✅ Changed ConductorConfig.pickup_radius_km to Optional[float] (forces DB config)
  - ✅ Added ConductorConfig.validate() method
  - ✅ Verified GPS telemetry safety (separate flow, no conflicts)
  - ✅ Analyzed coordinate validation (REMOVED - unnecessary with validated GTFS data)
  
  **Architecture Now:**
  Conductor → CommuterServiceClient → HTTP API → (Future: Reservoir) → Repository → Strapi
  
  **Estimated Time:** 3 hours remaining (7 phases)
  
  
  ### Phase 1: Shared Geospatial Utilities (30 min)
  - [ ] 1.1: Create arknet_transit_simulator/utils/geospatial.py
    - Consolidate: haversine(), bearing(), forward_point()
    - Add comprehensive docstrings with examples
    - Include type hints and validation
  - [ ] 1.2: Update imports in vehicle/conductor.py
    - Replace local haversine with: from arknet_transit_simulator.utils.geospatial import haversine
  - [ ] 1.3: Update imports in vehicle/driver/navigation/math.py
    - Import: haversine, bearing, forward_point
    - Remove local implementations
  - [ ] 1.4: Update imports in commuter_service/interfaces/http/passenger_crud.py
    - Replace local haversine with shared version
  - Success: All files use shared geospatial utilities, no duplicates
  
  ### Phase 2: Complete DB-Bound Configuration (15 min)
  - [ ] 2.1: Update ConductorConfig.from_config_service() in vehicle/conductor.py
    - Remove: pickup_radius_km or 0.2 fallback
    - Add: config.validate() call after loading
    - Test: Startup fails if pickup_radius_km not in DB
  - Success: Conductor requires DB config, no silent fallbacks
  
  ### Phase 4: PostGIS Integration for Passenger Queries (60 min) ⚠️ CRITICAL - PERFORMANCE
  **Why Critical:** Conductor performs real-time searches during operation. Current O(n) Python filtering
  becomes unusable at scale (500ms with 1k passengers, 5s with 10k passengers). PostGIS provides O(log n)
  spatial indexed queries: <15ms even with 10k passengers (416x speedup).
  
  - [ ] 4.1: Create /api/spatial/passengers/within-radius endpoint in geospatial_service
    - File: geospatial_service/api/routes.py
    - Method: POST /api/spatial/passengers/within-radius
    - Body: {lat, lon, radius_km, route_id (optional)}
    - Query: ST_DWithin(geography, point, radius) with spatial index
    - Return: [{passenger_id, lat, lon, distance}] sorted by distance
  - [ ] 4.2: Update /api/passengers/nearby in commuter_service
    - Call geospatial_service instead of Python haversine filtering
    - Fallback to Python if geospatial_service unavailable
    - Log: "🚀 PostGIS query" vs "🐌 Python fallback"
  - [ ] 4.3: Test PostGIS query performance
    - Seed 1000 passengers
    - Benchmark 100 queries
    - Verify: <15ms average per query
  - Success: Database-level spatial queries, 10x-400x speedup depending on passenger count
  
  ### Phase 5: Route Geometry Awareness (45 min)
  - [ ] 5.1: Add route_geometry attribute to Conductor
    - File: arknet_transit_simulator/vehicle/conductor.py
    - Add: self.route_geometry: Optional[List[Tuple[float, float]]] = None
  - [ ] 5.2: Update Driver to pass route geometry to conductor
    - File: arknet_transit_simulator/vehicle/driver/navigation/vehicle_driver.py
    - On route assignment: conductor.set_route_geometry(self.route)
  - [ ] 5.3: Enhance PostGIS query to use route corridor
    - Update geospatial_service endpoint to accept route_linestring parameter
    - Use: ST_Distance(linestring, point) < corridor_width
    - Prevents picking up passengers off-route (e.g., opposite side of highway)
  - Success: Conductor aware of route, searches along corridor not just circular radius
  
  ### Phase 6: Real-Time Configuration Updates (30 min)
  - [ ] 6.1: Add WebSocket listener in conductor
    - File: arknet_transit_simulator/vehicle/conductor.py
    - Subscribe to: operational-configuration updates from Strapi
    - On update: self.pickup_radius_km = new_value
    - Log: "⚙️ Config updated: pickup_radius_km {old} → {new}"
  - [ ] 6.2: Test dynamic config update
    - Change pickup_radius_km in Strapi
    - Verify conductor updates without restart
  - Success: Operators tune pickup radius without restarting fleet
  
  ### Phase 7: Integration Testing (30 min)
  - [ ] 7.1: End-to-end test with DB config
    - Verify conductor starts with DB pickup_radius_km
    - Verify conductor fails if config missing
  - [ ] 7.2: Performance test with 1000 passengers
    - Verify PostGIS queries <15ms
    - Verify GPS telemetry still works
  - [ ] 7.3: Route geometry test
    - Verify corridor filtering excludes off-route passengers
  - [ ] 7.4: Dynamic config test
    - Update pickup_radius_km in Strapi
    - Verify conductor updates in real-time
  - Success: All features working, performance targets met
  
  ### Phase 8: Documentation Cleanup (15 min)
  - [ ] 8.1: Update arknet_transit_simulator/README.md
    - Document new geospatial utilities
    - Document PostGIS integration
    - Document route geometry awareness
  - [ ] 8.2: Update commuter_service/README.md
    - Document /api/passengers/nearby performance characteristics
  - [ ] 8.3: Add inline docstrings
    - geospatial.py functions
    - PostGIS endpoint
  ...
  **Success Criteria:**
  - ✅ Conductor uses HTTP API (not direct database)
  - ✅ Shared geospatial utilities (no duplicates)
  - ✅ DB-bound configuration (no hardcoded fallbacks)
  - ✅ PostGIS spatial queries (<15ms with 10k passengers)
  - ✅ Route geometry awareness (corridor search)
  - ✅ Real-time config updates (no restart required)
  - ✅ GPS telemetry still works (verified separate flow)
  - ✅ All tests passing
  
  **Files Modified:**
  - ✅ arknet_transit_simulator/services/commuter_http_client.py (NEW)
  - ✅ arknet_transit_simulator/vehicle/conductor.py (refactored)
  - ✅ arknet_transit_simulator/simulator.py (updated conductor init)
  - ✅ commuter_service/interfaces/http/passenger_crud.py (added /api/passengers/nearby)
  - 🎯 arknet_transit_simulator/utils/geospatial.py (NEW - Phase 1)
  - 🎯 geospatial_service/api/routes.py (Phase 4)

TIER 4.10: Real-Time Event-Driven Manifest Streaming API (Deferred)
