#!/usr/bin/env python3
"""Test dispatcher authenticated SSE telemetry access"""
import requests
import json
import time

print("=" * 70)
print("Testing DISPATCHER Authenticated Telemetry Access via SSE")
print("=" * 70)

# Test with dispatcher authentication
print("\nTesting AUTHENTICATED access (dispatcher)...")
try:
    # Login to get JWT cookie
    print("Logging in as dispatcher...")
    login_response = requests.post(
        'http://localhost:7000/login',
        json={'username': 'dispatcher', 'password': 'Ga25w123'}
    )
    ...
    if login_response.status_code == 200:
        ...
        if response.status_code == 200:
            print("[OK] Connected to SSE stream with dispatcher authentication")
                    ...
                    if decoded.startswith('data:'):
                        ...
                        try:
                            ...
                            if obj.get('type') == 'update':
                                ...
                                if 'extras' in state:
                                    print(f"  [RESTRICTED] extras field present: {state.get('extras') is not None}")
                                else:
                                    print(f"  [OK] extras field NOT present (dispatcher should not see this)")
                                print()
                        except Exception as e:
                            print(f"  Parse error: {e}")
            
            print(f"\n[OK] Received {count} telemetry updates as dispatcher")
            print("[OK] Dispatcher should see deviceId, driverId but NOT extras field")
        else:
