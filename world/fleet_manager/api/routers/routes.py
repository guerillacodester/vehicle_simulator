...
from ..schemas.route import Route, RouteCreate, RouteUpdate, RoutePublic, RoutePublicCreate, RoutePublicUpdate, RoutePublicWithGeometry
...
@router.post("/private", response_model=Route)
def create_route(
    route: RouteCreate,
    db: Session = Depends(get_db)
):
    """Create a new route - PRIVATE API with full database access"""
...
@router.get("/private", response_model=List[Route])
def read_routes_private(
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db)
):
    """Get all routes with full data including UUIDs - PRIVATE API for admin use"""
